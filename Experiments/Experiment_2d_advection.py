...
class MLPConv(nn.Module):
    def __init__(self, sizes, noise=None, seed=0, fs=7, activation=nn.ELU()):
        ...
        for i in range(len(sizes)-1):
            ...
            # linear.weight.data=torch.tensor([[0.0,0.0,-20.0,0.0,0.0,0.0,20.0]])
...
def make_subplot_graphs_2d(NN_sim, v_coarse, x_sim, T_sim, T, dtc, n,
                        ...
                        dpi=800):
    """
    Строит графики на сетке  для заданных данных.
    ...
    n_times = len(time_lst)
    fig, axes = plt.subplots(n_times, 3, figsize=(figsize[0], figsize[1]*n_times),dpi=dpi)  # 3 столбца: точное, численное, разность
