...
from torch.optim import Adam,SGD,LBFGS,Adamax,RMSprop
...
main_params=pd.read_excel('experiment_init_params_Burgers_2_easy_params.xlsx',dtype={'has_backward':bool})
...
class MLPConv(nn.Module):
    ...
    def __init__(self, sizes, noise=None, seed=0, fs=7, activation=nn.ELU()):
        ...
        for i in range(len(sizes)-1):
            ...
            # nn.init.xavier_normal_(linear.weight, gain=1000)


            #---new data (256 & 16001)---
            # linear.weight.data=torch.tensor([[32.94204983171507,
            #                                 -65.88409966343013,
            #                                 32.94204983171507,
            #                                 0.0,
            #                                 0.0,
            ...
            #--------sabsampling=1 & only 6 coeffs--------
            # linear.weight.data=torch.tensor([[32.94204983171507,
            #                                 -65.88409966343013,
            #                                 32.94204983171507,
            #                                 0.0,
            #                                 20.292255244216655,
            #                                 -20.292255244216655]], requires_grad=True)


            #--------sabsampling=5--------
            # linear.weight.data=torch.tensor([[1.3176819932686026,
            #                                 -2.635363986537205,
            #                                 1.3176819932686026,
            #                                 0.0,
            #                                 0.0,
    ...
    def _preprocess(self, x):
        ...
        if self.fs%2!=0:
            for i in range(1, int(self.fs/2)+1):
                ...
                #only 9
                # px = torch.cat([l, px, r,l*px,px*px,r*px,l*l,r*r,r*l], -1) # объединяет 3 тензора

                #only 6
                px = torch.cat([l, px, r,px*px,l*l,r*r], -1)
...
def backward_rk1_error(net, target, dt, m, wd, 
    ...
    if bc_type=='dirichlet' and bc_values!=[None,None]:
        ...
        for j in range(1,m):
            ...
            res   = res + wd[j+1]*((target[m-(j+1):-(j+1),1:-1] - (p_new[:,1:-1] ))**2) #+ noise[m-(j+1):-(j+1),:]
            p_old = p_new  
...
def train_net(MLPConv,v_coarse_train,epochs,dtc,
    ...
    temp_net_layer=[fs*POLYNOM_COEF]+[neurons for i in range(hidden_layers_num)]+[1]
    ...
    if continue_fitting==None:
        ...
        net=continue_fitting[0].to(device)
        
    params    = [{'params': net.parameters(), 'lr': lr}]
    optimizer = Adam(params) # лучше
    # optimizer = LBFGS(params) # todo
    # optimizer = SGD(params) # плохо
    ...
    for epoch in pbar:
        ...
        #     # loss = criterion(outputs, targets)  # loss is a tensor
        #     loss.backward()
        #     return loss  # Return the tensor, don't call it!
...
def train_net_lbfgs(MLPConv, v_coarse_train, epochs, dtc,
              fs,
              neurons,
              hidden_layers_num,
              lr,
    ...
    for epoch in pbar:
        ...
        loss = optimizer.step(closure)  # передаем closure в step
        ...
        with torch.no_grad():
            ...
            for i in range(len(net.layer)):
                ...
                W = W.view(W.shape[0]*W.shape[1], -1)
                res_w = res_w + (torch.norm(W, p=2, dim=0)**2)
            current_loss = fwd + bwd + 0*res_w
            
        loss_lst.append([fwd.cpu().data.numpy(), bwd.cpu().data.numpy(),
                        l_wd*res_w.cpu().data.numpy()[0], current_loss.cpu().data.numpy()[0]])

        if epoch > 15_000:
            scheduler.step()

        if verbose and ((continue_fitting and (continue_fitting[1]+epoch)%verbose_step==0) or 
                       (not continue_fitting and epoch%verbose_step==0)):
            print(f'Веса после {continue_fitting[1]+epoch if continue_fitting else epoch} эпохи:')
            print(W)

        pbar.set_postfix(loss=round(current_loss.item(), 7))

    return net, loss_lst, current_loss
...
def make_simulation(net,v_coarse,L,Lxc,dtc,method='RK3',
    ...
    for j in tqdm(range(0,T_sim-1)):
        if method=='RK3':
            ...
            if bc_type=='dirichlet' and bc_values!=[None,None]:
                NN_sim[0,:]=bc_values[0]
                NN_sim[-1,:]=bc_values[-1]
            elif bc_type=='periodic':
                pass
            else:
                raise AssertionError('This bc_type didnt exist!')
