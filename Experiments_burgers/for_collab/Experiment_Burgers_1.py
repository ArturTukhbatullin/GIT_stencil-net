...
main_params=pd.read_excel('drive/MyDrive/diplom/Burgers/experiment_init_params_Burgers_1RK3_AuthorsParams.xlsx',dtype={'has_backward':bool})
...
def view_result_imshow(NN_sim, v_coarse, T, dtc, L, dxc,train_split,val_split,\
    ...
    plt.axvline(x=int(val_split * v_coarse.shape[1]), color='green', linestyle='-', linewidth=2)
...
def view_result_metric(NN_sim,v_coarse,T,dtc,L,dxc,train_split,val_split,
                       figsize=(7,6),n_xticks=2,
    save_path=None,
    save_name=None,
    ):
    ...
    plt.xlabel('t')
    plt.ylabel('MAE')
    plt.legend(['train_val_split','val_test_split','MAE'])
    plt.grid()
    if save_path!=None and save_name!=None:
